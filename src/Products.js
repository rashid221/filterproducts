...
const Products = () => {
  ...
  return (
    <div>
      <h4 style={{textAlign:'center',backgroundColor:'#212A3E',padding:'1rem',color:'white'}}>Products Availability</h4>
        ...
        <div style={{flex:1}}>
         {datas.map((item) => (
            <ul key={item.id}  style={{listStyleType:'none'}}>
                <li className="li-hover" onClick={() => dispatch(sendData(item.category))} >{item.category}</li>
            </ul>
          ))}
          </div>
       }
